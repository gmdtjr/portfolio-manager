...
def generate_exploration_prompt(investment_idea: str, exclusions: str = "") -> str:
    ...
    # 마스터 프롬프트 템플릿
    master_prompt_template = f"""# 유망 산업 및 종목 발굴 보고서
...
## **세부 분석 요건:**
1.  **산업 분석:** 각 유망 산업의 시장 규모(TAM), 연평균 성장률(CAGR), 주요 성장 동력 및 핵심 리스크 요인을 심층 분석할 것.
2.  **종목 분석:** 각 핵심 종목의 주요 비즈니스 모델, 기술적 해자, 재무 건전성, 그리고 현재 밸류에이션 수준을 종합적으로 분석할 것.
{exclusion_section}
"""
