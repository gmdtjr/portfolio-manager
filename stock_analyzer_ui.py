...
def render_stock_analyzer_page():
    """종목 상세 분석기 페이지 렌더링 (DB 연동 버전)"""
    
    st.title("🔬 종목 상세 분석기 (DB 연동)")
    st.markdown("분석할 종목을 입력하면, **'투자 노트' DB의 유무에 따라 맞춤형 분석 프롬프트**를 생성합니다.")
    ...
    with st.expander("📖 사용법 안내", expanded=False):
        ...
        **2단계: 프롬프트 생성**
        ...
        - 시스템이 자동으로 투자 노트 DB를 확인합니다
        
        **3단계: 맞춤형 분석**
        - **DB에 정보가 있는 경우**: 나의 기존 투자 아이디어를 검증하는 맞춤형 프롬프트 생성
        - **DB에 정보가 없는 경우**: 일반적인 종목 분석 프롬프트 생성
        ...
        **4단계: Deep Research 활용**
        ...
        ### 🎯 맞춤형 검증 프롬프트 (DB 연동 시)
        1. **투자 아이디어의 현실성 검증** - 나의 가설이 여전히 타당한가?
        2. **촉매 및 리스크 발생 현황 분석** - 예상했던 촉매/리스크가 현실화되고 있는가?
        3. **종합 재평가 및 결론** - 투자 아이디어 유지/수정/폐기 결정
