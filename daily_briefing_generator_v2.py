...
class DailyBriefingGeneratorV2:
    ...
    def get_macro_summary(self) -> str:
        """Gemini API를 사용하여 오늘의 매크로 이슈 요약 가져오기"""
        if not self.client:
            return "Gemini API가 설정되지 않았습니다."
        
        try:
            today = datetime.now().strftime('%Y년 %m월 %d일')
            macro_prompt = f"""오늘 날짜 기준({today}), 글로벌 금융 시장에 가장 큰 영향을 미칠 매크로 이슈 5가지를 각각 2줄로 요약해 줘"""
    ...
    def generate_complete_prompt(self) -> str:
        ...
        try:
            ...
            # 매크로 이슈 요약 가져오기
            macro_summary = self.get_macro_summary()
            
            # 완성된 프롬프트 생성
            complete_prompt = f"""📊 포트폴리오 데일리 브리핑 분석 요청 ({today})
🎯 Mission (임무)
당신은 월스트리트 최고의 포트폴리오 전략가입니다. 아래 제공되는 **[오늘의 글로벌 매크로 현황]**과 첨부된 [포트폴리오 파일] 2개를 종합적으로 분석하여, 실행 가능한 데일리 브리핑을 생성해주세요.

[오늘의 글로벌 매크로 현황 (AI 요약)]
{macro_summary}
...
1. 매크로 환경과 포트폴리오의 연결고리 분석
오늘 매크로 현황의 **가장 중요한 변수(예: 금리, 유가)**가 내 포트폴리오의 각 섹터(예: 기술주, 에너지주)에 미칠 긍정적/부정적 영향을 분석해주세요.

특히, 투자_노트.csv에 기록된 나의 **'핵심 리스크'**가 오늘의 매크로 환경에 의해 촉발될 가능성은 없는지 집중적으로 점검해주세요.

2. 투자 아이디어 재검증 (Thesis Re-validation)
오늘의 매크로 상황을 고려했을 때, 나의 핵심 투자 아이디어들이 여전히 유효한지, 아니면 일부 수정이 필요한지 평가해주세요.

예를 들어, "경기 침체 우려"가 커지는 상황이라면, 나의 '경기순환주'에 대한 투자 아이디어는 재검토가 필요할 수 있습니다.

3. 성과 원인 분석 (Performance Attribution)
포트폴리오_현황.csv의 어제 성과 Top/Bottom 종목을 선정하고, 그 성과의 원인이 개별 종목의 이슈인지, 아니면 오늘의 매크로 흐름에 편승한 결과인지 분석해주세요.

📝 Output Format (결과물 형식)
1. Executive Summary (핵심 요약)
매크로 요약: 오늘 시장의 핵심 키워드는? (예: 인플레이션 우려 지속)

포트폴리오 영향: 이로 인해 내 포트폴리오에 예상되는 영향은?

오늘의 전략: 그래서 오늘 내가 취해야 할 핵심 전략은?

2. Macro Impact Analysis (매크로 영향 분석)
긍정적 영향: [오늘의 매크로 환경이 긍정적으로 작용할 종목/섹터와 그 이유]

부정적 영향: [오늘의 매크로 환경이 부정적으로 작용할 종목/섹터와 그 이유]
...
위 분석을 바탕으로, 오늘 내가 취해야 할 구체적인 행동(예: 특정 섹터 비중 조절, 현금 확보, 리스크 관리 강화 등)을 1~2가지 제안해주세요."""
    ...
    def generate_complete_package(self) -> dict:
        ...
        try:
            ...
            # 4. 완성된 프롬프트 생성
            print("🤖 완성된 프롬프트 생성...")
            complete_prompt = self.generate_complete_prompt()
